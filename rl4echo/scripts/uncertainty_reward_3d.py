...
path = '../../3d_anatomical_reward_LM+ANAT_RNET1_TEMPSCALED_NEW_NARVAL_POLICY_CARDINAL.h5'
output = Path('unc_results_RNET+LM_TScale')

# path = '../../3dUNC_Ensemble_baseline3dunet.h5'
# output = Path('unc_results_Ensemble')

# path = '../../3dUNC_TTA_baseline3dunet.h5'
# output = Path('unc_results_TTA')

# path = '../../3dUNC_MCDropout_baseline3dunet.h5'
# output = Path('unc_results_MCDropout')

# path = '../../3dUNC_2dMICCAIRNET.h5'
# output = Path('unc_results_RNET_TScale_2D')
...
def ece(confidences, accuracies, nb_bins=20):
    ...
    np.save(f"{output}/bins_conf_w_size.npy", np.stack((bins_avg_conf, bins_avg_acc, bins_size)))
...
with h5py.File(path, "r") as h5:
    ...
    for key in h5.keys():
        ...
        ids.append(str(key))
        img = np.array(h5[key]['img']).transpose((1, 0, 2))
        pred = np.array(h5[key]['pred']).transpose((1, 0, 2))
        gt = np.array(h5[key]['gt']).transpose((1, 0, 2))
        if h5[key].get("reward_map+LM", None):
            reward_map = np.array(h5[key]['reward_map+LM']).transpose((1, 0, 2))
            uncertainty_map = 1 - reward_map
        else:
            reward_map = 1 - np.array(h5[key]['unc_map']).transpose((1, 0, 2))
            uncertainty_map = reward_map
        accuracy = np.array(h5[key]['accuracy_map']).transpose((1, 0, 2))
        error = 1 - accuracy
        ...
        ax3.imshow(pred[..., 0], cmap='gray')
        ax4.imshow(error[..., 0], cmap='gray')
        ax5.imshow(uncertainty_map[..., 0], cmap='gray')
        ...
        print(accuracy.shape, reward_map.shape)
        ...
        weighted_mean = np.sum(uncertainty_map.squeeze()) / np.sum(mask)
