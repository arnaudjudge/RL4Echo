...
class UNetACHeads(nn.Module):
    ...
    def __init__(
        ...
        self.critic_last_upsample = copy.deepcopy(self.upsamples[-1])
        self.critic_output_block = self.get_output_block(decoder_level=0, out_channels=1)

        self.params = nn.ModuleDict({
            'actor': nn.ModuleList([self.input_block, self.downsamples,
                                    self.bottleneck, self.upsamples, self.output_block]),
            'critic': nn.ModuleList([self.critic_last_upsample, self.critic_output_block])})

        self.apply(self.initialize_weights)

    def forward(
        self, input_data: Union[Tensor, MetaTensor], with_critic=False
        ...
        for i, zipped in enumerate(zip(self.upsamples, reversed(encoder_outputs))):
            upsample, skip = zipped
            if i == len(encoder_outputs)-1:
                out_critic = self.critic_last_upsample(out, skip)
            out = upsample(out, skip)
        out = self.output_block(out)
        out_critic = self.critic_output_block(out_critic)
        if with_critic:
            return out, out_critic
        return out
    ...
    def get_output_block(self, decoder_level: int, out_channels=None) -> nn.Module:
        ...
        return OutputBlock(
            ...
            out_channels=self.num_classes if out_channels is None else out_channels,
            dim=self.dim,
