...
class PolicyGradient(pl.LightningModule):
    ...
    def compute_policy_loss(self, batch, sample=True, **kwargs):
        ...
        return loss, reward, torch.zeros_like(loss), actions
    ...
    def validation_step(self, batch, batch_idx: int):
        ...
        self.log_tb_images((batch[0][0, ...].unsqueeze(0), actions[0, ...].unsqueeze(0), reward[0, ...].unsqueeze(0),
                            batch[1][0, ...].unsqueeze(0), batch[2][0, ...].unsqueeze(0), batch[4][0, ...].unsqueeze(0),
                            batch_idx))
    ...
    def log_tb_images(self, viz_batch, prefix="") -> None:
        ...
        # tb_logger.add_image(f"Prediction", torch.tensor(viz_batch[1][idx].cpu().detach().numpy()).unsqueeze(0), viz_batch[6])
        #
        # tb_logger.add_image(f"Previous prediction", torch.tensor(viz_batch[3][idx].squeeze(0).cpu().detach().numpy()).unsqueeze(0), viz_batch[6])
...
if __name__ == "__main__":
    ...
    logger = TensorBoardLogger('logs', name='policy_gradient')
